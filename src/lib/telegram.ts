...
// Импортируем тип ReactionEmoji для проверки
...
function cleanMessageText(text: string): string {
  ...
  // --- Заменяем рег. выражение на обратно-совместимое ---
      ...
      if (message.text) {
        ...
        if (cleanedText) {
          ...
          if (message.reactions && message.reactions.results.length > 0) {
            ...
            // --- Добавляем проверку типа реакции ---
            reactionDetails = message.reactions.results.reduce((acc, r) => {
              // Проверяем, что r.reaction существует и является классом ReactionEmoji
              if (r.reaction && r.reaction instanceof Api.ReactionEmoji) {
                // Только теперь мы можем безопасно получить .emoticon
                acc.push({
                  emoji: r.reaction.emoticon,
                  count: r.count,
                });
              }
              // Если это не ReactionEmoji, мы просто пропускаем эту реакцию
              return acc;
            }, [] as TelegramMessage['reactions']); // Начальное значение - пустой типизированный массив
          }
